...
class compareSpec {
  ...
  @test
  "should return 0 if inputs have different lenghts"() {
    expect(compare(compareSpec.a, compareSpec.d)).to.eq(0);
  }

  @test
  "should return 1 if inputs are equal"() {
    expect(compare(compareSpec.a, compareSpec.b)).to.eq(1);
  }

  @test
  "should return 0 if inputs are not equal"() {
    expect(compare(compareSpec.a, compareSpec.c)).to.eq(0);
  }

  @test
  "should return 1 if given zero-length inputs "() {
...
class EqualSpec {
  ...
  @test
  "should return false if inputs have different lenghts"() {
    expect(equal(EqualSpec.a, EqualSpec.d)).to.eq(false);
  }

  @test
  "should return true if inputs are equal"() {
    expect(equal(EqualSpec.a, EqualSpec.b)).to.eq(true);
  }

  @test
  "should return false if inputs are not equal"() {
    expect(equal(EqualSpec.a, EqualSpec.c)).to.eq(false);
  }

  @test
  "should return false if given zero-length inputs "() {
