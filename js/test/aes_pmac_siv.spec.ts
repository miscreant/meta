...
class AesPmacSivSpec {
  ...
  async "should correctly seal and open with polyfill cipher implementations"() {
    ...
    for (let v of AesPmacSivSpec.vectors) {
      const siv = await AesSiv.importKey(
        polyfillProvider,
        "AES-PMAC-SIV",
        v.key
      );
  ...
  async "should not open with incorrect associated data"() {
    ...
    for (let v of AesPmacSivSpec.vectors) {
      ...
      return expect(siv.open(v.ciphertext, badAd)).to.be.rejectedWith(
        IntegrityError
      );
    }
  }
  ...
  async "should not open with incorrect ciphertext"() {
    ...
    for (let v of AesPmacSivSpec.vectors) {
      ...
      return expect(siv.open(badOutput, v.ad)).to.be.rejectedWith(
        IntegrityError
      );
    }
  }
